...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_hid, n_position=200):
        ...
        # 该组参数的特别之处在于：模型训练时不会更新（即调用 optimizer.step() 后该组参数不会变化，只可人为地改变它们的值），但是保存模型时，该组参数又作为模型参数不可或缺的一部分被保存。
...
class PositionalEncoding_sim(nn.Module):

    def __init__(self, d_hid, n_position=200):
        super(PositionalEncoding_sim, self).__init__()

        # Not a parameter
        self.register_buffer('pos_table', self._get_sinusoid_encoding_table(n_position, d_hid))

    def _get_sinusoid_encoding_table(self, n_position, d_hid):
        ''' cosine-similarity position encoding table '''
